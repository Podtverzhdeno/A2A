...
6. A3 создаёт один order ID и один payment draft ID.
7. REST API, MCP tools и Deal Ledger возвращают состояние и события.
...
6. Сделка переходит в `order_created`.
7. Создаются `order_id` и `payment_draft_id`.
...
Ожидаемый статус: `order_created`. Повторное подтверждение возвращает тот же
`order_id` и `payment_draft_id`, не создавая второй заказ.
